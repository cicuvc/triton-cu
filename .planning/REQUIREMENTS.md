...
- [x] **INFER-03**: Inference runs at IR-build (semantic) time. The `ttg.extern_call` op result type and all downstream consumers (`gl.store`, arithmetic, etc.) stay type-consistent — no MLIR verification failures after lowering.
